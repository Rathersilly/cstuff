// NOTE: spdlog usage example
// - By default, spdlog uses header only library (slow compile time)
//
// NOTE: To use static archive (/usr/local/lib/spdlog.a):
// #define SPDLOG_COMPILED_LIB
// #include "spdlog/spdlog.h"
// - compile with g++ foo.cc -lspdlog
//
// NOTE: Or define it for whole project:
// g++ foo.cpp -SPDLOG_COMPILED_LIB -lspdlog -o foo
//
...
void callback_example() {
  // Create the logger
  auto logger = spdlog::callback_logger_mt(
      "custom_callback_logger", [](const spdlog::details::log_msg & /*msg*/) {
...
void binary_example() {
  ...
  // spdlog::to_hex(buf)); logger->info("hexdump style, 20 chars per line {:a}",
  // spdlog::to_hex(buf, 20));
}
#else
void binary_example() {
...
void replace_default_logger_example() {
  ...
  auto new_logger =
      spdlog::basic_logger_mt("new_default_logger", "logs/somelog.txt", true);
...
// Mapped Diagnostic Context (MDC) is a map that stores key-value pairs (string
// values) in thread local storage. Each thread maintains its own MDC, which
// loggers use to append diagnostic information to log outputs. Note: it is not
// supported in asynchronous mode due to its reliance on thread-local storage.
...
void mdc_example() {
  ...
  // if not using the default format, you can use the %& formatter to print mdc
  // data as well
