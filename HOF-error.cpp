...
// Best practice: the callee should throw an exception, and the caller
// should handle it. The callee may catch only if it can resolve the problem
// itself.
double divide(double a, double b) {
  ...
  if (b == 0) {
    ...
    // stack until it finds a matching catch, or the program terminates.
    // This process ("stack unwinding") destroys all local objects as usual
    // by calling their destructors. Beware: it may skip over important
    // cleanup if you're using raw new/delete, so prefer RAII types
    // (std::unique_ptr, std::vector, etc.).
