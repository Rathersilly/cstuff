// A method of creating a callback for a particular function
// using a macro - not sure if it will prove useful
...
public:
  ...
  int operator()(int one, int two) {
    std::cout << "Reporter: FunctionName() is being called by " << caller_
              << "() in " << file_ << ":" << line_ << std::endl;
